 ...
 * what killed the VMM outright when this driver was written.  A device without
...
/*
 * Find the host bridge's configuration space: its base address and the first
 * bus number it covers.
 *
 * Both descriptions of a machine are handled, because neither is guaranteed to
 * be there.  A device tree names the window in a "pci-host-ecam-generic" node;
 * firmware that describes the machine only by ACPI puts the same thing in the
 * MCFG, and a platform whose console is a PCI function may well be one with no
 * device tree at all.
 */
static bool __init vtcon_ecam_window(paddr_t *base, unsigned int *bus)
{
    *bus = 0;
    ...
    if ( !acpi_disabled )
    {
        ...
        if ( ACPI_FAILURE(acpi_get_table(ACPI_SIG_MCFG, 0,
                                         (struct acpi_table_header **)&table)) )
        {
            printk(XENLOG_WARNING "vtcon: no MCFG table to search\n");
            return false;
        }

        mcfg = container_of(table, const struct acpi_table_mcfg, header);
        if ( table->length < sizeof(*mcfg) + sizeof(*alloc) )
        {
            printk(XENLOG_WARNING "vtcon: MCFG describes no allocation\n");
            return false;
        }

        /* The first segment only: the console is on the bus the firmware used. */
        alloc = (const struct acpi_mcfg_allocation *)(mcfg + 1);
        *base = alloc->address;
        *bus = alloc->start_bus_number;
 ...
 * uart_init(), which runs after vm_init() so that ioremap() works and before
...
{
    unsigned int bar, nbar, notify_cap, bus;
    uint32_t off, len, noff, nlen;
    paddr_t ecam_base, barbase;
    void __iomem *ecam;

    if ( !vtcon_ecam_window(&ecam_base, &bus) )
        return;
