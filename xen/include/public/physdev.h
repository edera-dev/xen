 ...
 * device's MSI is composed by the host, not by the domain, so the domain has to
 * name a vector the host may deliver to - and the VP the host delivers to is
 * Xen's, not the domain's.  The domain therefore composes every vPCI MSI with
 * this vector and receives the resulting interrupts as VIRQ_HYPERV_VPCI on the
 * vcpu whose VP the host targeted, demultiplexing among its own devices.
 *
 * Fails with -ENODEV if Xen is not relaying Hyper-V enlightenments to this
 * domain.
 */
...
typedef struct physdev_hyperv_vpci_vector physdev_hyperv_vpci_vector_t;
DEFINE_XEN_GUEST_HANDLE(physdev_hyperv_vpci_vector_t);
