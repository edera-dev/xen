    ...
    unsigned long start_extent)
{
    ...
    /* A run has to be one: wrapping it would name frames twice. */
    if ( contiguous && (xcr.start_gfn + xcr.nr_frames < xcr.start_gfn) )
        return -EINVAL;
     ...
     */
    if ( xcr.nr_frames > (UINT_MAX >> MEMOP_EXTENT_SHIFT) ||
         start_extent > xcr.nr_frames )
        return -EINVAL;

    rc = rcu_lock_remote_domain_by_id(xcr.domid, &d);
    if ( rc )
        return rc;
    ...
    to_guest = !(xcr.flags & XENMEM_DOMAIN_COPY_from_guest);

    for ( done = start_extent; done < xcr.nr_frames; done++ )
    {
        xen_pfn_t gfn;
        struct page_info *page;
        p2m_type_t p2mt;
        ...
        if ( contiguous )
            gfn = xcr.start_gfn + done;
        else if ( copy_from_guest_offset(&gfn, xcr.gfns, done, 1) )
        {
            rc = -EFAULT;
            goto out;
        }
...
long do_memory_op(unsigned long cmd, XEN_GUEST_HANDLE_PARAM(void) arg)
{
 ...
 vnumainfo_out:
    ...
    case XENMEM_domain_copy_range:
        rc = domain_copy_range(
            guest_handle_cast(arg, xen_domain_copy_range_t),
            start_extent);
        break;
