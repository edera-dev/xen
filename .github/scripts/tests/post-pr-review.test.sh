...
reset_state
check "unknown section rejected" bash -c "! bash '$SCRIPT' edera-dev/xen 42 nope '$WORK/body-pr-test-coverage.md' 2>/dev/null"
check "non-numeric pr rejected" bash -c "! bash '$SCRIPT' edera-dev/xen abc pr-test-coverage '$WORK/body-pr-test-coverage.md' 2>/dev/null"
check "empty body rejected" bash -c "! bash '$SCRIPT' edera-dev/xen 42 pr-test-coverage /dev/null 2>/dev/null"
printf '<!-- section:pr-test-coverage -->\nx\n' >"$WORK/marked.md"
check "body with markers rejected" bash -c "! bash '$SCRIPT' edera-dev/xen 42 pr-test-coverage '$WORK/marked.md' 2>/dev/null"
