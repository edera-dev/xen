...
publish() {
  ...
  bash "$SCRIPT" edera-dev/xen 42 "$section" "$WORK/body-$section.md" "$HEAD_SHA"
}
...
reset_state
FAKE_GH_USER_TYPE=User FAKE_GH_USER_LOGIN=someone gh api -X POST repos/edera-dev/xen/pulls/42/reviews -f event=COMMENT -f body='<!-- pr-review --> mine' >/dev/null
...
same=$(bash "$SCRIPT" edera-dev/xen 42 pr-review-suggestions "$WORK/unfinished.md" "$HEAD_SHA" --only-if-unstamped 2>/dev/null)
...
bash "$SCRIPT" edera-dev/xen 42 pr-review-suggestions "$WORK/unfinished.md" "$HEAD_SHA" --only-if-unstamped >/dev/null
...
id=$(bash "$SCRIPT" edera-dev/xen 42 pr-review-suggestions "$WORK/unfinished.md" "$newer" --only-if-unstamped)
...
check "an unknown option is rejected" \
  bash -c "! bash '$SCRIPT' edera-dev/xen 42 pr-review-suggestions '$WORK/unfinished.md' '$newer' --nope 2>/dev/null"

echo "# the head sha is required and must be a sha"
reset_state
printf 'text\n' >"$WORK/body-args.md"
check "missing head sha is rejected" \
  bash -c '! bash "'"$SCRIPT"'" edera-dev/xen 42 pr-review-suggestions "'"$WORK"'/body-args.md"'
check "non-hex head sha is rejected" \
  bash -c '! bash "'"$SCRIPT"'" edera-dev/xen 42 pr-review-suggestions "'"$WORK"'/body-args.md" not-a-sha'
...
reset_state
export FAKE_GH_AFTER_LIST_HOOK="printf '%s\n' '<!-- pr-review -->' '' '## PR Review' '<!-- section:pr-review-suggestions -->' 'Other check got there first.' '<!-- /section:pr-review-suggestions -->' '' '## Test Coverage' '<!-- section:pr-test-coverage -->' '_This check has not posted for this pull request yet._' '<!-- /section:pr-test-coverage -->' > '$WORK/other.md' && gh api -X POST repos/edera-dev/xen/pulls/42/reviews -f event=COMMENT -F body=@'$WORK/other.md' >/dev/null"
id=$(publish pr-test-coverage "Coverage text.")
unset FAKE_GH_AFTER_LIST_HOOK
...
check "unknown section rejected" bash -c "! bash '$SCRIPT' edera-dev/xen 42 nope '$WORK/body-pr-test-coverage.md' '$HEAD_SHA' 2>/dev/null"
check "non-numeric pr rejected" bash -c "! bash '$SCRIPT' edera-dev/xen abc pr-test-coverage '$WORK/body-pr-test-coverage.md' '$HEAD_SHA' 2>/dev/null"
...
check "empty body rejected" bash -c "! bash '$SCRIPT' edera-dev/xen 42 pr-test-coverage /dev/null '$HEAD_SHA' 2>/dev/null"
printf '<!-- section:pr-test-coverage -->\nx\n' >"$WORK/marked.md"
check "body with a section marker rejected" bash -c "! bash '$SCRIPT' edera-dev/xen 42 pr-test-coverage '$WORK/marked.md' '$HEAD_SHA' 2>/dev/null"
printf 'text\n<!-- stamp -->\n' >"$WORK/stamped.md"
check "body with a stamp marker rejected" bash -c "! bash '$SCRIPT' edera-dev/xen 42 pr-test-coverage '$WORK/stamped.md' '$HEAD_SHA' 2>/dev/null"
printf 'text\n<!-- pr-review -->\n' >"$WORK/review-marked.md"
check "body with the review marker rejected" bash -c "! bash '$SCRIPT' edera-dev/xen 42 pr-test-coverage '$WORK/review-marked.md' '$HEAD_SHA' 2>/dev/null"
...
for wf in "${WORKFLOWS[@]}"; do
  ...
  check "$name: same branches" grep -qF "    branches: ['edera/**']" "$wf"
